<template>
  <div>
    ...
    <el-dialog v-model="visibleDept" width="30%" :modal="false" :title="placeholder" :close-on-click-modal="false" :close-on-press-escape="false">
      ...
      <el-tree class="filter-tree" :data="deptList" :default-expanded-keys="expandedKeys" :props="{ label: 'deptName', children: 'children' }" :expand-on-click-node="false" :filter-node-method="filterNode" :highlight-current="true" node-key="id" ref="treeRef"> </el-tree>
...
const commitHandle = () => {
  ...
  emit("update:deptName", node.deptName);
  showDeptName.value = node.deptName;
  visibleDept.value = false;
  deptList.value = [];
  filterText.value = "";
};
