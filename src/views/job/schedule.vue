...
const view = reactive({
  getDataListURL: "/system/schedule/page",
  getDataListIsPage: true,
  deleteURL: "/system/schedule",
  deleteIsBatch: true,
  dataForm: {
    beanName: ""
  }
...
const pauseHandle = (id?: string) => {
  ...
  ElMessageBox.confirm("确定进行[暂停]操作?", "提示", {
    ...
    .then(() => {
      baseService.put("/system/schedule/pause", id ? [id] : state.dataListSelections && state.dataListSelections.map((item: IObject) => item.id)).then((res) => {
        ElMessage.success({
          message: "成功",
          duration: 500,
          onClose: () => {
...
const resumeHandle = (id?: string) => {
  ...
  ElMessageBox.confirm("确定进行[恢复]操作?", "提示", {
    ...
    .then(() => {
      baseService.put("/system/schedule/resume", id ? [id] : state.dataListSelections && state.dataListSelections.map((item: IObject) => item.id)).then((res) => {
        ElMessage.success({
          message: "成功",
          duration: 500,
          onClose: () => {
...
const runHandle = (id?: string) => {
  ...
  ElMessageBox.confirm("确定进行[执行]操作?", "提示", {
    ...
    .then(() => {
      baseService.put("/system/schedule/run", id ? [id] : state.dataListSelections && state.dataListSelections.map((item: IObject) => item.id)).then((res) => {
        ElMessage.success({
          message: "成功",
          duration: 500,
          onClose: () => {
