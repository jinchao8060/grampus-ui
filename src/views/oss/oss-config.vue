    ...
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="140px">
      ...
      <template v-else-if="dataForm.type === 2">
        <el-form-item prop="aliyunDomain" label="域名">
          <el-input v-model="dataForm.aliyunDomain" placeholder="阿里云绑定的域名，如：http://cdn.oceancloud.com"></el-input>
        </el-form-item>
...
// 获取信息
const getInfo = () => {
  baseService.get("/system/oss/info").then((res) => {
...
const dataFormSubmitHandle = () => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    baseService.post("/system/oss", dataForm).then((res) => {
      ElMessage.success({
        message: "成功",
        duration: 500,
        onClose: () => {
