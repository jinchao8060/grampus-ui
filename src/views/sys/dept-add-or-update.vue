    ...
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="parentName" label="上级部门" class="dept-list">
        <el-popover :width="400" ref="deptListPopover" placement="bottom-start" trigger="click" popper-class="popover-pop">
          ...
          <div class="popover-pop-body"><el-tree :data="deptList" :props="{ label: 'deptName', children: 'children' }" node-key="id" ref="deptListTree" :highlight-current="true" :expand-on-click-node="false" accordion @current-change="deptListTreeCurrentChangeHandle"> </el-tree></div>
        </el-popover>
      </el-form-item>
...
const getInfo = (id: number) => {
  baseService.get(`/system/dept/${id}`).then((res) => {
...
const deptListTreeCurrentChangeHandle = (data: IObject) => {
  ...
  dataForm.parentName = data.deptName;
...
const dataFormSubmitHandle = () => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    (!dataForm.id ? baseService.post : baseService.put)("/system/dept", dataForm).then((res) => {
      ElMessage.success({
        message: "成功",
        duration: 500,
        onClose: () => {
