  ...
  <el-dialog v-model="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="rules" ref="dataFormRef" @keyup.enter="dataFormSubmitHandle()" label-width="120px">
      ...
      <el-form-item prop="menuName" label="名称">
        <el-input v-model="dataForm.menuName" placeholder="名称"></el-input>
      </el-form-item>
          ...
          <template v-slot:reference>
            <el-input v-model="dataForm.parentName" :readonly="true" placeholder="上级菜单">
              <template v-slot:suffix>
                <el-icon v-if="dataForm.parentId !== '0'" @click.stop="deptListTreeSetDefaultHandle()" class="el-input__icon"><circle-close/></el-icon>
              </template>
            </el-input>
          </template>
          <div class="popover-pop-body">
            <el-tree :data="menuList" :props="{ label: 'menuName', children: 'children' }" node-key="id" ref="menuListTree" :highlight-current="true" :expand-on-click-node="false" accordion @current-change="menuListTreeCurrentChangeHandle"> </el-tree>
          </div>
        </el-popover>
      </el-form-item>
      <el-form-item v-if="dataForm.type === 0" prop="url" label="路由">
        <el-input v-model="dataForm.path" placeholder="路由"></el-input>
      </el-form-item>
...
const init2 = (row: IObject) => {
  ...
  dataForm.parentName = row.menuName;
...
// 获取菜单列表
const getMenuList = () => {
  return baseService.get("/system/menu/list?type=0").then((res) => {
...
const getInfo = (id: number) => {
  baseService.get(`/system/menu/${id}`).then((res) => {
...
const menuListTreeCurrentChangeHandle = (data: IObject) => {
  ...
  dataForm.parentName = data.menuName;
...
const dataFormSubmitHandle = () => {
  dataFormRef.value.validate((valid: boolean) => {
    ...
    (!dataForm.id ? baseService.post : baseService.put)("/system/menu", dataForm).then((res) => {
      ElMessage.success({
        message: "成功",
        duration: 500,
        onClose: () => {
