...
export const useAppStore = defineStore("useAppStore", {
  ...
  actions: {
    ...
    initApp() {
      return Promise.all([
        baseService.get("/system/menu/nav"), //加载菜单
        baseService.get("/system/menu/permissions"), //加载权限
        baseService.get("/system/user/info"), //加载用户信息
        baseService.get("/system/dict/all") //加载字典
      ]).then(([menus, permissions, user, dicts]) => {
        if (user.status !== 200) {
          console.error("初始化用户数据错误", user.message);
        }
